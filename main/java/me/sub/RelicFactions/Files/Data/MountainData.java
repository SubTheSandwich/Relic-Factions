...
public class MountainData {
    ...
    public static MountainData getByName(String name) {
        File[] mountains = new File(Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("Relic-Factions")).getDataFolder().getPath() + "/data/mountains").listFiles();
        if (mountains != null) {
            for (File f : mountains) {
                YamlConfiguration file = YamlConfiguration.loadConfiguration(f);
                if (Objects.requireNonNull(file.getString("name")).equalsIgnoreCase(name.toLowerCase())) {
                    return new MountainData(UUID.fromString(Objects.requireNonNull(file.getString("uuid"))));
                }
            }
        }
        return null;
    ...
    public static File[] getAll() {
        return new File(Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("Relic-Factions")).getDataFolder().getPath() + "/data/mountains").listFiles();
    }
